...
PATH = "SchisMTV Wizard"            
BASEURL = "http://schismtv.pcriot.com/repo"
H = 'http://'
skin         =  xbmc.getSkinDir()
EXCLUDES     = ['backupdir','plugin.video.schismwizard','script.module.addon.common','repository.schismtv.addons','backup','backup.zip']
...
zip = 'special://home/addons/plugin.video.schismwizard'
urlbase      =  'None'
...
USB          =  xbmc.translatePath(os.path.join('special://home/addons/plugin.video.schismwizard',''))
...
def BACKUP():  
    ...
    if choice == 0:
	 return
      ...
      exclude_dirs_full =  ['backupdir','plugin.video.schismwizard','repository.schismtv.addons','script.skinshortcuts','plugin.program.super.favourites']
 ...
 if os.path.exists(os.path.join(backupdir,'addon_data.zip')):   
	import time
        ...
        if choice == 0:
			...
			dp.create("[COLOR=blue][B]SchisM TV[/B][/COLOR] Custom Builds Tool","Restoring",'', 'Please Wait')
...
def WipeXBMC():
    if skin!= "skin.confluence":
        dialog.ok('[COLOR=blue][B]SchisM TV[/B][/COLOR][COLOR=green][/COLOR] Custom Builds Tool','Please switch to the default Confluence skin','before performing a wipe.','')
        ...
        if choice == 1:
            mybackuppath = xbmc.translatePath(os.path.join(backupdir,'SchisM TV Builds','My Builds'))
            ...
            exclude_dirs_full =  ['backupdir','plugin.video.schismwizard','repository.schismtv.addons']
            exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    ...
    if choice2 == 0:
        ...
        dp.create("[COLOR=blue][B]SchisM TV[/B][/COLOR] Custom Builds Tool","Wiping Install",'', 'Please Wait')
        try:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('[COLOR=blue][B]SchisMTV [/B][/COLOR] Custom Builds Tool','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
...
def WipeInstall():
    if skin!= "skin.confluence":
        dialog.ok('[COLOR=blue][B]SchisM TV[/B][/COLOR][COLOR=green]box[/COLOR] Tool','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")       
    else:
